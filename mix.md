...
a [base65536]() hash encrypted text.
...
The first one is [Shift Keyboard]() encrypted data as the next highlited word from the challenge description says.
...
The middle one are some decimal number presentation of the middle part of the flag.
First I have turned the [decimals into hex]() format & after that I turned the [hex into text]() format.
...
The third one is [base64]() encoded data. After decoding this I got another cipher text which looks like some 
binary data but that is a [Spoon]() cipher as the last highlited word from the challenge description says
